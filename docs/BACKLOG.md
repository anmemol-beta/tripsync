...
### 5. `[x]` HITL: ambiguous-ask clarifying question
...
### 10. `[x]` mongodb-mcp-server self-host config
...
### 11. `[x]` MongoDB indices + schema validation
...
- **2026-05-21** Item 2: Activity decision type — `mockSearchActivities` (5 canned candidates) added to `runtime.ts`; `searchActivities` now uses it by default; `test/activity-path.test.ts` (6 cases: two full propose→vote→decide cycles, `decisions.activities` has 2 entries). 27/27 tests green.
...
- **2026-05-20** Item 10: mongodb-mcp-server self-host config — `deploy/mcp/Dockerfile` (node:22-alpine, `@mongodb-js/mongodb-mcp-server`, `MDB_MCP_ALLOWED_TOOLS=find,insert-one,update-one,aggregate`, HTTP transport on 8080), `deploy/mcp/cloudrun.yaml` (authenticated Cloud Run service, Secret Manager for Atlas URI, scale 0-2), `deploy/mcp/README.md` (7 concrete steps: vars → SA → secret → build → deploy → invoker → Agent Builder). `pnpm test` 15/15 green.
- **2026-05-20** Item 11: MongoDB indices + schema validation — `packages/schema/src/indexes.ts` (`applyIndexes` declares 3 indices: proposals on (trip_id,kind,status), votes unique on (proposal_id,voter), history on (trip_id,created_at)); `MemCollection.createIndex` + unique enforcement added to test/utils/memdb.ts; `test/indexes.test.ts` (3 cases: apply runs, duplicate (proposal_id,voter) rejected with E11000, different proposal_id allowed). 18/18 tests green.
