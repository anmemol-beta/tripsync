...
| **2:25–2:45** | seo taps "Create travel video". The agent creates a 60-second vertical recap brief with 3 scenes: decision montage, voting conflict, Tokyo arrival. UI shows the video job and scenes. | "Planning is not the ending. The final artifact is the trip video. Gemini turns the MongoDB trip memory into a concrete recap brief and stores it as a video job." | `find_trip`, `create_travel_video`, `append_history` |
| **2:45–2:55** | Architecture diagram (from `ARCHITECTURE.md` §1). Highlight: Gemini 3 → Agent Builder → MongoDB MCP → MongoDB → video job. | "Gemini 3 drives the loop. Vertex AI Agent Builder hosts and observes. MongoDB is both shared memory and the source of truth for the recap-video pipeline." | — |
